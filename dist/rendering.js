      ...
      var newInstance = function newInstance(element) {
        ...
        chart.option = {
          ...
          legend: {
            orient: 'vertical',
            left: 'right',
            data: [{ name: 'iPhone 7', icon: 'roundRect', textStyle: { color: '#f4e925' } }, { name: 'iPhone 8', icon: 'roundRect', textStyle: { color: '#f4e925' } }, { name: 'iPhone X', icon: 'roundRect', textStyle: { color: '#f4e925' } }],
            selectedMode: 'single',
            selected: {
              'iPhone 7': true,
              'iPhone 8': false,
        ...
        chart.update = function () {
          var selectedInfo = chart.option.legend.selected;

          for (var selKey in selectedInfo) {
          ...
          if (typeof ctrl.city !== 'undefined') {
            ...
            chart.option.series[0].data = ctrl.popData.iphone7.filter(function (x) {
              return x.parent === ctrl.province;
            });
            chart.option.series[1].data = ctrl.popData.iphone8.filter(function (x) {
              return x.parent === ctrl.province;
            });
            chart.option.series[2].data = ctrl.popData.iphonex.filter(function (x) {
              return x.parent === ctrl.province;
            });
          } else {
